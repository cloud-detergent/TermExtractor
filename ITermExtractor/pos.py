...
class Tagger:
    ...
    def get_label(self, id):
        tag = tag_inv[id] if id in tag_inv else '?'
        tag_enum = POSNameConverter.recognise_enum(tag)
        return tag_enum
    ...
    def label(self, sentence):
        """
        Определяет части речи каждого слова в предложении

        #'S' - 1.0, 'A' - 2.0
        >>> s = Tagger()
        >>> s.load("../tmp/svm.model", "../tmp/ids.pickle")
        >>> s.label("Огонь минометных батальонов")
        [('Огонь', 1.0), ('минометных', 2.0), ('батальонов', 1.0)]
        >>> s.label("Основная, задача")
        [('Основная', 2.0), ('задача', 1.0)]
...
class TaggerFeatures:
    def __init__(self):
        ...
        self._train = True
        self._featurespace = FeatureSpace()
